...
function 手臂() {
  ...
  rotate(angle.x, 8000);
  画包络线(主体颜色, 手臂起始点, 手臂各点位移);
}

function draw() {
  背景色(白色);
  画包络线(主体颜色, 身体起始点, 身体各点位移);
  填色椭圆(脸色, {x: 360, y: 500, 宽: 200, 高: 270})
  for (i in 所有眉毛) {
    画包络线(眉毛颜色, 眉毛起始点[所有眉毛[i]], 眉毛各点位移[所有眉毛[i]]);
  }
  // 左右眼
  填色椭圆(主体颜色, {x: 300, y: 465, 宽: 20 + mouseX / 26, 高: 20})
  填色椭圆(主体颜色, {x: 403, y: 458, 宽: 20 + mouseX / 26, 高: 20})

  嘴()
  手臂()
  // 手指
  填色椭圆(主体颜色, {x: 170, y: 560, 宽: mouseX / 64, 高: mouseX / 21})
}